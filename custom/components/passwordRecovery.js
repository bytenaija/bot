...
module.exports = {
  ...
  invoke: (conversation, done) => {
    const tableCreationQuery = 'CREATE TABLE IF NOT EXISTS `password_recovery` ( `transID` INT NOT NULL AUTO_INCREMENT , `email` VARCHAR(255) NOT NULL , `SystemType` VARCHAR(255) NOT NULL , `code` INT(6) NOT NULL , `level` VARCHAR(255) NULL , PRIMARY KEY (`transID`)) ENGINE = MyISAM';
    ...
    if (SystemType == 'National Joint Qualification System (NJQS) (Qualification system)') {
      ...
      const { QuestionnaireType } = conversation.properties();
      // 80.241.219.166
      // db db_nipex_dnb
      // user nipex_staging
      //  pass N1p2e3x4#
